    ...
    <section className="relative">
      <div className="flex items-center justify-between mb-8">
        <div className="flex items-center gap-4">
          <div className="w-12 h-12 rounded-[20px] bg-green-500/10 flex items-center justify-center text-green-500 border border-green-500/20 shadow-lg shadow-green-500/5">
            <FileCheck size={24} />
          </div>
          <div>
            <h2 className="text-2xl font-black text-white tracking-tight uppercase">Abgeschlossene Inspektionen</h2>
            <p className="text-zinc-500 text-[10px] font-bold uppercase tracking-widest">{completed.length} Berichte</p>
          </div>
        </div>
      </div>
        ...
        <AnimatePresence mode="popLayout">
          {currentCompleted.map((report, idx) => (
            <motion.div
              ...
              className="glass-premium relative p-6 rounded-[2.5rem] flex flex-col md:flex-row items-center justify-between gap-6 hover:bg-white/[0.03] transition-all border border-white/5 active:scale-[0.99]"
            >
              <div className="flex items-center gap-6 flex-1 w-full">
                <div className="w-16 h-16 rounded-[24px] bg-green-500/10 border border-green-500/20 flex items-center justify-center text-green-500 shadow-xl shadow-green-500/5">
                  <FileCheck size={32} />
                </div>
                <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-x-12 gap-y-4 flex-1">
                  <div className="space-y-1">
                    <p className="text-[10px] text-zinc-500 uppercase font-black tracking-widest">Standort</p>
                    <p className="text-zinc-100 font-black uppercase text-sm tracking-tight">{report.ort}</p>
                  </div>
                  <div className="space-y-1">
                    <p className="text-[10px] text-zinc-500 uppercase font-black tracking-widest">Kunde</p>
                  ...
                  <div className="space-y-1">
                    <p className="text-[10px] text-zinc-500 uppercase font-black tracking-widest">Datum</p>
              ...
              <div className="flex items-center gap-3 w-full md:w-auto">
                <Link href={`/inspection/${report._id}/review`} className="flex-1 md:flex-none">
                  <button className="w-full flex items-center justify-center gap-2 px-8 py-4 rounded-2xl bg-white/5 border border-white/5 hover:bg-white/10 text-white font-black uppercase text-[10px] tracking-widest transition-all cursor-pointer shadow-xl">
                    <Eye size={18} />
                    Anzeigen
                  </button>
                </Link>
                <div className="flex-1 md:flex-none flex gap-3">
                  <Link href={`/inspection/${report._id}/review`} className="flex-1">
                    <button className="w-full h-full flex items-center justify-center gap-2 px-8 py-4 rounded-2xl bg-green-500/10 border border-green-500/20 hover:bg-green-500 text-green-500 hover:text-black font-black uppercase text-[10px] tracking-widest transition-all cursor-pointer shadow-xl">
                      <Download size={18} />
                    </button>
                  </Link>
                  <button 
                    onClick={(e) => handleDeleteTrigger(report._id, e)}
                    className="p-4 rounded-2xl bg-red-500/10 border border-red-500/20 text-red-500 hover:bg-red-500 hover:text-white transition-all cursor-pointer shadow-xl transition-colors"
                  >
                    <Trash2 size={18} />
                  </button>
                </div>
